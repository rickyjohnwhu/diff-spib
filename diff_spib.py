...
class GaussianFourierProjection(nn.Module):
    """
    Gaussian random Fourier feature mapping.

    Parameters
    ----------
    embed_dim : int, default=4
        The output embedding dimension (must be even).
    scale : float, default=100.0
        Scaling factor applied to the sampled frequencies.

    Attributes
    ----------
    W : torch.nn.Parameter
        Fixed random frequencies used for Fourier projection.
    """
    def __init__(self, embed_dim=4, scale=100.):
    ...
    def forward(self, x):
        ...
        x : torch.Tensor of shape (batch_size,)
            Input tensor of scalar values.

        Returns
        -------
        torch.Tensor of shape (batch_size, embed_dim)
            The sinusoidal embedding of the inputs.
        """
...
class ScoreNet(nn.Module):
    """
    A neural network for score-based diffusion models.

    Combines raw input, time embeddings, and temperature embeddings
    to predict added noise in a diffusion generative process.

    Parameters
    ----------
    embed_dim : int
        Dimension of the Gaussian Fourier embeddings for time and temperature.
    """
    def __init__(self, embed_dim):
        ...
        self.tlin1 = nn.Linear(embed_dim, 16)
        self.templin1 = nn.Linear(embed_dim, 16)
        self.lin2 = nn.Linear(16, 64)
        self.tlin2 = nn.Linear(embed_dim, 64)
        self.templin2 = nn.Linear(embed_dim, 64)
        self.lin3 = nn.Linear(64, 64)
        self.tlin3 = nn.Linear(embed_dim, 64)
        self.templin3 = nn.Linear(embed_dim, 64)
        self.lin4 = nn.Linear(64, 64)
        self.tlin4 = nn.Linear(embed_dim, 64)
        self.templin4 = nn.Linear(embed_dim, 64)
        self.lin5 = nn.Linear(64, 64)
        self.tlin5 = nn.Linear(embed_dim, 64)
        self.templin5 = nn.Linear(embed_dim, 64)
        self.lin6 = nn.Linear(64, 16)
        self.tlin6 = nn.Linear(embed_dim, 16)
        self.templin6 = nn.Linear(embed_dim, 16)
        self.lin7 = nn.Linear(16, 2)
        self.tlin7 = nn.Linear(embed_dim, 2)
        self.templin7 = nn.Linear(embed_dim, 2)
    ...
    def forward(self, x, t, temp):
        ...
        t : torch.Tensor of shape (batch_size,)
            Time indices for diffusion steps.
        temp : torch.Tensor of shape (batch_size,)
            Temperature embeddings.

        Returns
        -------
        torch.Tensor of shape (batch_size, 2)
            Predicted noise for denoising diffusion.
        """
        embed = self.act(self.embed(t))
        temp_embed = self.act(self.temp_embed(temp))

        h1 = self.act(self.lin1(x) + self.tlin1(embed) + self.templin1(temp_embed))
        h2 = self.act(self.lin2(h1) + self.tlin2(embed) + self.templin2(temp_embed))

        h3 = self.act(self.lin3(h2) + self.tlin3(embed) + self.templin3(temp_embed) + h2)
        h4 = self.act(self.lin4(h3) + self.tlin4(embed) + self.templin4(temp_embed) + h3)
        h5 = self.act(self.lin5(h4) + self.tlin5(embed) + self.templin5(temp_embed) + h4)

        h6 = self.act(self.lin6(h5) + self.tlin6(embed) + self.templin6(temp_embed))
        h7 = self.lin7(h6)
        return h7
...
class TiltGauss:
    ...
    def sample(self, n):
        ...
        indices = torch.searchsorted(self.cdf, u, right=True)
        radii = self.r_vals[indices]
...
class DSPIB(nn.Module):
    """
    A D-SPIB model which can be fit to data optimizing for dimension reduction and MSM.
    Parameters
    ----------
    output_dim : int
    ...
    custom_var : float, default=1
        Multiplier for noise in diffusion model.
    timesteps_per_sample : int, default=8
        Number of times to randomly sample and train for a given training datapoint.
    tau : float, default=1.0
        Parameter for tilted Gaussian prior.
    """
    def __init__(self, output_dim, data_shape, encoder_type='Nonlinear', z_dim=2, lagtime=1, beta=1e-3,
                 ...
                 custom_var=1, timesteps_per_sample=8, tau=1.0):

        super(DSPIB, self).__init__()
        ...
        self.score_net = ScoreNet(16)
        self.score_net = self.score_net.to(self.device)

        self.MSE = nn.MSELoss()

        self.T = 100

        self.beta_arr = torch.linspace(1e-4, 0.2, self.T).to(self.device)
        self.diff_t = torch.arange(0, self.T).to(self.device)
        self.alpha_t = 1 - self.beta_arr.to(self.device)
        self.alpha_bar_t = self.alpha_t.cumprod(0).to(self.device)

        self.custom_var = custom_var
        self.timesteps_per_sample = timesteps_per_sample

        self.tau = tau
        self.tilt_gauss_2 = TiltGauss(self.tau, 2.0/2.0, self.device)
        self.tilt_gauss_5 = TiltGauss(self.tau, 5.0/2.0, self.device)
    ...
    # Loss function
    # ------------------------------------------------------------------------------
    def calculate_loss(self, data_inputs, data_targets, data_weights, temperature_weights, diff_set=False):
        ...
        if diff_set==True:
            ...
            aug_x = torch.repeat_interleave(z_sample, repeats=self.timesteps_per_sample, dim=0)
            aug_temp = torch.repeat_interleave(temperature_weights, repeats=self.timesteps_per_sample, dim=0)

            samples_2 = self.tilt_gauss_2.sample(z_sample.shape[0]*self.timesteps_per_sample)
            samples_5 = self.tilt_gauss_5.sample(z_sample.shape[0]*self.timesteps_per_sample)
            epsilon = torch.where(aug_temp.view(-1, 1) == 1.0, samples_2 * 1.0, samples_5 * 2.5) * self.custom_var
              
            sel_t = torch.randint(100,(z_sample.shape[0]*self.timesteps_per_sample,), device=self.device)

            noised = torch.sqrt(self.alpha_bar_t[sel_t]).unsqueeze(-1) * aug_x + torch.sqrt(1 - self.alpha_bar_t[sel_t]).unsqueeze(-1) * epsilon
            preds = self.score_net(noised, sel_t, aug_temp)

            diff_loss = self.MSE(preds, epsilon) / 2

            kl_loss = torch.sum(data_weights*(log_q))/data_weights.sum()

        if diff_set==False:

            diff_loss = torch.tensor([0], device=self.device)
        ...
        loss = reconstruction_error + self.beta * kl_loss + self.beta * diff_loss

        return loss, reconstruction_error.detach().cpu().data, kl_loss.detach().cpu().data, diff_loss.detach().cpu()
    ...
    def update_model(self, inputs, input_weights, train_data_labels, test_data_labels, batch_size, threshold=0):
        ...
        for i in range(train_data_labels.shape[-1]):
            ...
            # find the one closest to center_z as representative-inputs
    ...
    def fit(self, train_dataset, test_dataset, diffusion=False, batch_size=128, tolerance=0.001, patience=5, refinements=15,
        ...
        while True:
            ...
            train_epoch_diff_loss = 0
            for batch_inputs, batch_outputs, batch_weights, batch_temperatures in train_dataloader:
                step += 1

                if diffusion:
                  loss, reconstruction_error, kl_loss, diff_loss = self.calculate_loss(batch_inputs, batch_outputs, batch_weights, 
batch_temperatures, diff_set=True)
                else:
                  loss, reconstruction_error, kl_loss, diff_loss = self.calculate_loss(batch_inputs, batch_outputs, batch_weights, batch_temperatures, diff_set=False)
                ...
                train_epoch_diff_loss += diff_loss * weight_sum
            ...
            with torch.no_grad():
                ...
                train_epoch_diff_loss /= weight_sum

                print(
                    "Epoch %i:\tTime %f s\nLoss (train) %f\tKL loss (train): %f\n"
                    "Reconstruction loss (train) %f\tDiffusion loss (train): %f" % (
                        epoch, train_time, train_epoch_loss, train_epoch_kl_loss, train_epoch_reconstruction_error, train_epoch_diff_loss))
                print(
                    "Epoch %i:\tTime %f s\nLoss (train) %f\tKL loss (train): %f\n"
                    "Reconstruction loss (train) %f\tDiffusion loss (train): %f" % (
                        epoch, train_time, train_epoch_loss, train_epoch_kl_loss,
                        train_epoch_reconstruction_error, train_epoch_diff_loss), file=open(log_path, 'a'))

                test_epoch_loss = 0
                test_epoch_kl_loss = 0
                test_epoch_reconstruction_error = 0
                test_epoch_diff_loss = 0
                for batch_inputs, batch_outputs, batch_weights, batch_temperatures in test_dataloader:

                    if diffusion:
                      loss, reconstruction_error, kl_loss, diff_loss = self.calculate_loss(batch_inputs, batch_outputs, batch_weights, batch_temperatures, diff_set=True)
                    else:
                      loss, reconstruction_error, kl_loss, diff_loss = self.calculate_loss(batch_inputs, batch_outputs, batch_weights, batch_temperatures, diff_set=False)
                    ...
                    test_epoch_diff_loss += diff_loss * weight_sum
                ...
                test_epoch_diff_loss /= weight_sum

                print(
                    "Loss (test) %f\tKL loss (test): %f\n"
                    "Reconstruction loss (test) %f\tDiffusion loss (test): %f" % (
                        test_epoch_loss, test_epoch_kl_loss, test_epoch_reconstruction_error, test_epoch_diff_loss))
                print(
                    "Loss (test) %f\tKL loss (test): %f\n"
                    "Reconstruction loss (test) %f\tDiffusion loss (test): %f" % (
                        test_epoch_loss, test_epoch_kl_loss, test_epoch_reconstruction_error, test_epoch_diff_loss), file=open(log_path, 'a'))
    ...
    def output_final_result(self, train_dataset, test_dataset, batch_size, index=0):
        ...
        with torch.no_grad():
            ...
            final_result = []
            # output the result
            loss, reconstruction_error, kl_loss, diff_loss = [0 for i in range(4)]

            for batch_inputs, batch_outputs, batch_weights, batch_temperatures in train_dataloader:

                loss1, reconstruction_error1, kl_loss1, diff_loss1 = self.calculate_loss(batch_inputs, batch_outputs, batch_weights, batch_temperatures, diff_set=True)
                ...
                diff_loss += diff_loss1 * batch_weights.sum()
            ...
            diff_loss /= weight_sum

            final_result += [loss.data.cpu().numpy(), reconstruction_error.cpu().data.numpy(), kl_loss.cpu().data.numpy(), diff_loss.cpu().data.numpy()]
            print(
                "Final: %d\nLoss (train) %f\tKL loss (train): %f\n"
                        "Reconstruction loss (train) %f\tDiffusion loss (train): %f" % (
                    index, loss, kl_loss, reconstruction_error, diff_loss))
            print(
                "Final: %d\nLoss (train) %f\tKL loss (train): %f\n"
                        "Reconstruction loss (train) %f\tDiffusion loss (train): %f" % (
                    index, loss, kl_loss, reconstruction_error, diff_loss),
            ...
            final_result += [loss.cpu().data.numpy(), reconstruction_error.cpu().data.numpy(), kl_loss.cpu().data.numpy(), diff_loss.cpu().data.numpy()]
            print(
                "Loss (test) %f\tKL loss (test): %f\n"
                "Reconstruction loss (test) %f\tDiffusion loss (test): %f"
                % (loss, kl_loss, reconstruction_error, diff_loss))
            print(
                "Loss (test) %f\tKL loss (test): %f\n"
                "Reconstruction loss (test) %f\tDiffusion loss (test): %f"
                % (loss, kl_loss, reconstruction_error, diff_loss), file=open(summary_path, 'a'))
...
class NewTimeLaggedDataset(torch.utils.data.Dataset):
    """
    Dataset class designed to prepare input data for D-SPIB training;
